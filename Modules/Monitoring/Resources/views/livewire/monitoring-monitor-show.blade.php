...
<script>
    ...
    function renderMonitorChart() {
        ...
        const data = getMonitorChartData();
        el.innerHTML = '';
        new ApexCharts(el, {
            chart: { type: 'area', height: 260, toolbar: { show: false }, animations: { enabled: false }, foreColor: obioraDarkTheme() ? '#94a3b8' : '#64748b' },
            theme: { mode: obioraDarkTheme() ? 'dark' : 'light' },
            series: [{ name: 'Response ms', data: data.values || [] }],
            xaxis: { categories: data.categories || [], labels: { show: false } },
            yaxis: { labels: { formatter: (v) => v + ' ms' } },
            stroke: { curve: 'smooth', width: 2 },
            markers: { size: 4, colors: ['#3b82f6'], strokeWidth: 2, strokeColors: '#3b82f6', hover: { size: 6 } },
            colors: ['#3b82f6'],
            fill: { type: 'gradient', gradient: { opacityFrom: 0.35, opacityTo: 0.05 } },
            dataLabels: { enabled: false },
            grid: { borderColor: obioraDarkTheme() ? 'rgba(148,163,184,0.15)' : 'rgba(15,23,42,0.08)' },
            tooltip: { theme: obioraDarkTheme() ? 'dark' : 'light', x: { show: true } },
        }).render();
    }

    function waitAndRenderMonitor(attempts = 0) {
        if (typeof ApexCharts !== 'undefined' && document.getElementById('monitor-response-chart')) {
            renderMonitorChart();
        } else if (attempts < 50) {
            setTimeout(() => waitAndRenderMonitor(attempts + 1), 120);
        }
    }

    $wire.on('$refresh', () => setTimeout(waitAndRenderMonitor, 80));
    document.addEventListener('livewire:navigated', () => waitAndRenderMonitor());
    waitAndRenderMonitor();
</script>
@endscript
